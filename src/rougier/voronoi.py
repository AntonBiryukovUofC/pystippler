...
def rasterize_outline(V):
    ...
    n = len(V)
    X, Y = V[:, 0], V[:, 1]
    ymin = np.int(np.ceil(Y.min()))
    ymax = np.int(np.floor(Y.max()))
    size_pts = (2 + (ymax - ymin) * 2, 3)
    points = np.zeros(size_pts, dtype=np.int64)
    index = 0
    for y in range(ymin, ymax + 1):
        segments = []
        for i in range(n):
        ...
        segments.sort()
        for i in range(0, (2 * (len(segments) // 2)), 2):
            x1 = np.int64(np.ceil(segments[i]))
            x2 = np.int64(np.ceil(segments[i + 1]))
            points[index, :] = np.array([x1, x2, y])
            index += 1
    return points[:index]
...
def filter_regions(vor, bbox, epsilon):
    regions = []
...
def filter_regions_fast(vor, bbox, epsilon):
    regions = []
    vertices = vor.vertices

    # regions_orig = np.array(vor.regions, dtype=np.int16)
    regions_orig = vor.regions
    for region in regions_orig:
        if len(region) > 0:
            res = _filter_region(region, bbox, epsilon, vertices=vertices)
            if res is not None:
                regions.append(res)
    return regions


def _filter_region(region, bbox, epsilon, vertices):
    flag = True
    for index in region:
        if index == -1:
            flag = False
...
def voronoi(points, bbox):
    ...
    # Filter regions
    epsilon = 0.1
    regions = filter_regions(vor, bbox, epsilon)
